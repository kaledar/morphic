...
export class Yet2OpenAIAssistantLanguageModel implements LanguageModelV1 {
  ...
  private static auxAssistantThreadId: string | null = null
  private static auxAssistantRunId: string | null = null
    ...
    const stream = new ReadableStream<LanguageModelV1StreamPart>({
      start: async controller => {
        try {
          ...
          console.log(
            `doStream mode input tools details: ${JSON.stringify(toolDetails)}`
          )

          /*
          if (mode.type === 'object-json') {
            Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId = null
            Yet2OpenAIAssistantLanguageModel.mainAssistantRunId = null
          }
            */

          /*
          if (
            Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId &&
            Yet2OpenAIAssistantLanguageModel.mainAssistantRunId
          ) {
            console.log(
              `doStream: there is an existing main assistant run for tool interaction`
            )
            ...
            if (runStatus.status === 'completed') {
              console.log(`assistant thread run completed...completed...`)
              controller.enqueue({
                type: 'finish',
                finishReason: 'stop',
          ...
          if (
            Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId &&
            Yet2OpenAIAssistantLanguageModel.auxAssistantRunId
          ) {
            console.log(
              `doStream: there is an existing auxillary assistant run for related queries or others`
            )
            const runStatus = await this.openai.beta.threads.runs.retrieve(
              Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId!,
              Yet2OpenAIAssistantLanguageModel.auxAssistantRunId!
            ...
            if (runStatus.status === 'completed') {
              console.log(
                `assistant thread auxillary run completed...completed...`
              )
              controller.enqueue({
                type: 'finish',
                finishReason: 'stop',
          ...
          // TODO: bu kisim gereksiz kaldi gorunuyor.
          ...
          if (assistantResponse.type === 'tool-call') {
            ...
            controller.enqueue({
              ...
              toolCallType: 'function',
              //toolCallId: toolCall.id,
              toolCallId: assistantResponse.toolCalls[0].id,
              toolName: assistantResponse.toolCalls[0].function.name,
              args: assistantResponse.toolCalls[0].function.arguments
            })
            //}
            */
          } /* else if (
            (assistantResponse.role === 'assistant' &&
              ...
              assistantResponse.content.type === 'text') ||
            mode.type === 'object-json'
          ) {
            console.log(`doStream: should stop the run`)
  ...
  private async runAssistant(messages: any[], tools: any[]): Promise<any> {
    ...
    if (!Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId) {
      const thread = await this.openai.beta.threads.create()
      Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId = thread.id
      console.log(
        `customModel: created new thread with id ${Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId}`
      )
    } else {
      console.log(
        `customModel: using existing thread with id ${Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId}`
      )
    }
    ...
    // First, check if we have a tool result to submit
    ...
    if (toolResultMessage) {
      ...
      try {
        ...
        // Check if it's actually a tool result
        if (parsedToolResult[0]?.type === 'tool-result') {
          ...
          toolResultToSubmit = {
            ...
            runId: Yet2OpenAIAssistantLanguageModel.mainAssistantRunId!,
    ...
    if (toolResultToSubmit) {
      ...
      if (tools.length > 0) {
        ...
        Yet2OpenAIAssistantLanguageModel.mainAssistantRunId = run.id
      } else {
        run = await this.openai.beta.threads.runs.create(
          Yet2OpenAIAssistantLanguageModel.mainAssistantThreadId!,
          {
        ...
        Yet2OpenAIAssistantLanguageModel.auxAssistantRunId = run.id
      }
    }
  ...
  private getToolsByCreating(): any[] {
    return [
      {
        type: 'function',
        function: {
  ...
  private getTools(mode: any): any[] {
    ...
    // Return an empty array if no tools are provided
    return []
  }
}
